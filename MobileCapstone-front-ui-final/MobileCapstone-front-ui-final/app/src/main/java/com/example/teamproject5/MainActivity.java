...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupListeners() {
        ...
        CheckBox checkboxWritten = findViewById(R.id.checkbox_written);
        CheckBox checkboxPractical = findViewById(R.id.checkbox_practical);
        CheckBox checkboxBoth = findViewById(R.id.checkbox_both);
        ...
        if (etSearch != null) {
            etSearch.setOnEditorActionListener((v, actionId, event) -> {
                if (actionId == android.view.inputmethod.EditorInfo.IME_ACTION_SEARCH || actionId == android.view.inputmethod.EditorInfo.IME_ACTION_DONE) {
                    ...
                    // 체크박스 값 수집
                    java.util.ArrayList<String> selectedForms = new java.util.ArrayList<>();
                    if (checkboxWritten != null && checkboxWritten.isChecked()) selectedForms.add("필기");
                    if (checkboxPractical != null && checkboxPractical.isChecked()) selectedForms.add("실기");
                    if (checkboxBoth != null && checkboxBoth.isChecked()) selectedForms.add("필기+실기");

                    // 쉼표 구조로 가공 ("필기,실기")
                    StringBuilder sb = new StringBuilder();
                    for (int i = 0; i < selectedForms.size(); i++) {
                        sb.append(selectedForms.get(i));
                        if (i < selectedForms.size() - 1) sb.append(",");
                    }
                    String formResult = sb.toString();

                    // 안전하게 전달
                    Intent intent = new Intent(MainActivity.this, CertificateListActivity.class);
                    intent.putExtra("searchQuery", searchQuery);
                    intent.putExtra("form", formResult);
